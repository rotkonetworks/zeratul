...
export function createRelayTransport(
  ...
  function connect(room: string, _nick: string) {
    // use random nick for relay - real identity exchanged after encryption
    currentNick = 'p' + crypto.getRandomValues(new Uint8Array(4)).reduce((s, b) => s + b.toString(16).padStart(2, '0'), '')
    isCreator = !room
  ...
  function doConnect(room: string) {
    const relayUrl = `wss://${location.host}/ws`
    ...
    switch (msg['t']) {
      ...
      case 'joined': {
        ...
        if (!hasJoined) {
          ...
          // send ephemeral X25519 pubkey only - no identity info to the relay.
          // identity (zafuPub, delegation, mode) is exchanged AFTER encryption.
          if (ephemeral) {
            if (sessionIdentity) {
    ...
    if (wireMsg.t === '_keyex') {
      ...
      if (ephemeral) {
        try {
          ...
          setEncrypted(true)
          onRoom('encrypted')
          // send identity through encrypted channel (never plaintext)
          if (sessionIdentity && !wasRekey) {
            send({ t: '_identity', d: {
              mode: sessionIdentity.mode,
              zafuPub: sessionIdentity.zafuPubKey,
              delegation: sessionIdentity.delegation,
              nick: sessionIdentity.nick,
            }})
          }
  ...
  function send(msg: WireMessage) {
    ...
    if (sessionKey) {
      ...
      // no crypto available - queue instead of sending plaintext
      // messages will be flushed after key exchange completes
      console.warn('[crypto] no session key or ephemeral key - queueing message')
      pendingMessages.push(msg)
    }
  }
