...
// ── FROST escrow (via zafu external API) ──

/** create a FROST multisig escrow wallet via zafu */
export async function frostCreate(
  zafu: { origin: string },
  opts: { threshold?: number; maxSigners?: number; relayUrl?: string } = {},
): Promise<{ success: boolean; address?: string; roomCode?: string; publicKeyPackage?: string; error?: string }> {
  try {
    const extId = zafu.origin.replace('chrome-extension://', '').replace(/\/$/, '')
    const resp: any = await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(extId, {
        type: 'zafu_frost_create',
        threshold: opts.threshold || 2,
        maxSigners: opts.maxSigners || 3,
        relayUrl: opts.relayUrl || 'https://zcash.rotko.net',
      }, (r: any) => {
        if (chrome.runtime.lastError) reject(chrome.runtime.lastError)
        else resolve(r)
      })
...
/** join an existing FROST DKG room via zafu */
export async function frostJoin(
  zafu: { origin: string },
  roomCode: string,
  opts: { threshold?: number; maxSigners?: number; relayUrl?: string } = {},
): Promise<{ success: boolean; address?: string; publicKeyPackage?: string; error?: string }> {
  try {
    const extId = zafu.origin.replace('chrome-extension://', '').replace(/\/$/, '')
    const resp: any = await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(extId, {
        type: 'zafu_frost_join',
        roomCode,
        threshold: opts.threshold || 2,
        maxSigners: opts.maxSigners || 3,
        relayUrl: opts.relayUrl || 'https://zcash.rotko.net',
...
/** co-sign a transaction with FROST key share via zafu */
export async function frostSign(
  zafu: { origin: string },
  roomCode: string,
  sighashHex: string,
  opts: { relayUrl?: string } = {},
): Promise<{ success: boolean; signed?: boolean; error?: string }> {
  try {
    const extId = zafu.origin.replace('chrome-extension://', '').replace(/\/$/, '')
    const resp: any = await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(extId, {
        type: 'zafu_frost_sign',
        roomCode,
        sighashHex,
        relayUrl: opts.relayUrl || 'https://zcash.rotko.net',
      }, (r: any) => {
