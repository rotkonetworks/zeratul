//! penumbra bridge pallet
//!
//! threshold signature bridge for penumbra using decaf377-frost.
//! signers participate in frost ceremonies to control penumbra custody addresses.
//!
//! ## design
//!
//! penumbra uses decaf377 curve with rdsa signatures (similar to zcash's reddsa).
//! decaf377-frost provides threshold signing compatible with penumbra's spendauth.
//!
//! ```text
...
//! ## signature types
//!
//! penumbra uses two signature types:
//! - spendauth: authorizes spending (threshold signed)
//! - binding: aggregates value commitments (deterministic from tx)
...
/// frost signature on decaf377 curve (penumbra rdsa)
#[derive(Clone, Encode, Decode, DecodeWithMemTracking, TypeInfo, MaxEncodedLen, Debug, PartialEq)]
pub struct PenumbraSignature {
    /// r component (commitment point, compressed)
    pub r: [u8; 32],
...
pub struct SigningRequest<AccountId, S: Get<u32>, P: Get<u32>> {
    ...
    /// penumbra transaction fragment to sign
    pub tx_data: BoundedVec<u8, S>,
    pub created_at: u32,
    pub deadline: u32,
    pub signer_fee: u64,
    pub committed_signers: BoundedVec<(u16, u32), P>,
    pub partial_sigs: BoundedVec<(u16, PenumbraSignature), P>,
    pub final_sig: Option<PenumbraSignature>,
...
pub enum BridgeState {
    ...
    EmergencyRecovery { initiated_at: u32, recovery_address: [u8; 80] },
}
...
/// penumbra address (80 bytes: diversifier + transmission key + clue key)
#[derive(Clone, Encode, Decode, DecodeWithMemTracking, TypeInfo, MaxEncodedLen, Debug, PartialEq)]
pub struct PenumbraAddress {
    pub inner: [u8; 80],
}

impl Default for PenumbraAddress {
    fn default() -> Self {
        Self { inner: [0u8; 80] }
    }
}

impl PenumbraAddress {
...
pub mod pallet {
    ...
    /// group public key - the penumbra spend authorization key
    #[pallet::storage]
    pub type GroupPublicKey<T: Config> = StorageValue<_, [u8; 32]>;

    /// full verification key (for address derivation)
    #[pallet::storage]
    pub type FullViewingKey<T: Config> = StorageValue<_, [u8; 64]>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// request signature on penumbra transaction
        #[pallet::call_index(4)]
        #[pallet::weight(10_000)]
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn compute_group_public_key() -> [u8; 32] {
            // in real impl: sum of public key shares on decaf377
            let signers = ActiveSignerList::<T>::get();
        ...
        fn aggregate_signatures(
            partials: &BoundedVec<(u16, PenumbraSignature), MaxPartialSigs>,
        ) -> Option<PenumbraSignature> {
            if partials.is_empty() {
                return None;
            }

            // in real impl: lagrange interpolation on decaf377
            let mut r = [0u8; 32];
            let mut s = [0u8; 32];
...
pub trait PenumbraBridgeInterface<AccountId> {
    fn is_bridge_active() -> bool;
    fn custody_address() -> Option<PenumbraAddress>;
...
impl<T: pallet::Config> PenumbraBridgeInterface<T::AccountId> for pallet::Pallet<T> {
    fn is_bridge_active() -> bool {
    ...
    fn custody_address() -> Option<PenumbraAddress> {
        pallet::GroupPublicKey::<T>::get().map(|pubkey| {
            // derive penumbra address from group pubkey
            // in real impl: use ivk derivation
            let mut addr = [0u8; 80];
            addr[..32].copy_from_slice(&pubkey);
            PenumbraAddress { inner: addr }
        })
    }
